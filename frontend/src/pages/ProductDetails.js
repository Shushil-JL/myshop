...
const ProductDetails = () => {
  ...
  const fetchProductDetails = async()=>{
    ...
    setActiveImage(dataReponse?.data?.productImage.url)
                          ...
                          // data?.productImage?.map((imgURL,index) =>{
                            // return(
                              <div className='h-20 w-20 bg-slate-200 rounded p-1'>
                                <img alt={data.productName} src={data.productImage.url} className='w-full h-full object-scale-down mix-blend-multiply cursor-pointer' onMouseEnter={()=>handleMouseEnterProduct(imgURL)}  onClick={()=>handleMouseEnterProduct(imgURL)}/>
                              </div>
                            // )
                          // })
                        }
